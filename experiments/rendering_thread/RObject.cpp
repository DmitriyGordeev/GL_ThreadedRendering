...
RObject::RObject(const glm::vec2& pos, const glm::vec2& size):
    m_Position(pos),
    m_WorldSize(size) {
    createGeometry();
}
...
void RObject::buildBuffers() {

    // 1. Create vertex array RObject
    if (m_VaoID == 0) {
        glGenVertexArrays(1, &m_VaoID);
        glBindVertexArray(m_VaoID);
    }

    // 2. Create vertex buffer RObject and bind our Vertex* data to its ID (m_VboID)
...
void RObject::applyShader(const std::shared_ptr<Shaders>& shader) {
    if (!shader) {
        Logger::info("RObject::applyShader() shader is nullptr");
        return;
    }

    if (shader->getShaderProgramId() == 0) {
        Logger::info("RObject::applyShader() shader program ID = 0");
        return;
    }
    ...
    if (m_VaoID == 0) {
        Logger::error("RObject::buildBuffer() seems wasn't called before applying a shader");
        return;
    }

    // Save reference to associated material for this RObject
    m_ShaderRef = shader;
...
void RObject::render() {
    if (!m_ShaderRef.lock()) {
        Logger::info("RObject::render() - shader is null");
        return;
    }
